...
Static-ish goal-state extraction from a Lean 4 file by querying the Lean LSP
endpoint `$/lean/plainGoal` across (line, col) positions and recording
goal-state transitions.

What it produces:
- trace.json: {
    file, uri,
    unique_states: {hash -> rendered_goal_text},
    occurrences: [
      {hash, line, col_start, col_end, sample_pos: {line, character}}
    ]
  }

Usage:
  uv run python lean_inspect.py path/to/File.lean
  uv run python lean_inspect.py path/to/File.lean --out trace.json
  uv run python lean_inspect.py path/to/File.lean --mode adaptive
  uv run python lean_inspect.py path/to/File.lean --lake /home/anand/.elan/bin/lake

Notes:
- LSP positions are 0-based (line, character).
- This does NOT rely on Lean widget RPC (which may be empty in your setup).
- It relies on `$/lean/plainGoal`, which you already confirmed works.
"""
...
async def wait_for_diagnostics(c: LspClient, uri: str, version: int) -> None:
    # Lean-specific; you already confirmed it's available.
...
def goal_key(goal: Optional[dict]) -> Optional[str]:
    if goal is None:
        return None
    # In your results, this is a ```lean ...``` fenced string; good enough to hash.
...
async def scan_line_adaptive(
    ...
    This is a good default for non-toy files.
    """
    n = len(line_text)
    if n == 0:
...
async def run(args: argparse.Namespace) -> None:
    file_rel = Path(args.file)
    if not file_rel.exists():
        print(f"error: file not found: {file_rel}", file=sys.stderr)
        raise SystemExit(2)

    text = file_rel.read_text(encoding="utf-8")
    lines = text.splitlines()

    uri = path_to_uri(file_rel)
    root_uri = path_to_uri(Path.cwd())

    lake_path = args.lake
    if lake_path is None:
        # Prefer PATH, but if not present, try ~/.elan/bin/lake
        if shutil_which("lake") is not None:
            lake_path = "lake"
        else:
            lake_path = str(Path.home() / ".elan" / "bin" / "lake")
    ...
    # Wait for elaboration/diagnostics to settle
    await wait_for_diagnostics(c, uri, version)

    unique_states: dict[str, str] = {}
    occurrences: list[Occurrence] = []

    scanner = scan_line_adaptive if args.mode == "adaptive" else scan_line_dense

    # If user only wants a small window (debug), allow limits
    ...
    for ln in range(start_line, end_line):
        ...
        for oc in occs:
            if oc.h not in unique_states:
                # Recover the rendered state text by looking at the transition start col
                # (We could store it earlier, but this keeps code simple.)
        ...
        if args.progress and (ln - start_line) % 20 == 0:
            print(f"scanned line {ln}/{end_line - 1}  uniques={len(unique_states)} occs={len(occurrences)}")
    ...
    if args.progress or args.print_summary:
        ...
        print(f"unique goal states: {len(unique_states)}")
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("file", help="Path to .lean file (relative or absolute)")
    ap.add_argument("--out", default="trace.json", help="Output JSON path")
    ap.add_argument(
        "--mode",
        choices=["dense", "adaptive"],
        default="dense",
        help="dense = query every column; adaptive = fewer queries with boundary refinement",
    )
    ap.add_argument("--lake", default=None, help="Path to lake binary (default: lake or ~/.elan/bin/lake)")
    ap.add_argument("--start-line", type=int, default=None, help="Scan starting at this 0-based line")
    ap.add_argument("--end-line", type=int, default=None, help="Scan up to (but not including) this 0-based line")
    ap.add_argument("--print-init", action="store_true", help="Print initialize result")
    ap.add_argument("--print-summary", action="store_true", help="Print a short summary at end")
    ap.add_argument("--progress", action="store_true", help="Print progress every ~20 lines")
